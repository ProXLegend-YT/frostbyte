/**
 * Built-in tool & skill catalog. These are prompt-level "skills" (system-prompt
 * snippets + behavior contracts the agent follows) and tool bindings (calls out
 * to Tavily/GitHub/etc). Users can enable/disable any of these, and add their
 * own custom tools from the Tools tab — everything here is just seed data
 * for a rich out-of-the-box library.
 */
...
export const TOTAL_TOOL_COUNT = TOOL_CATEGORIES.reduce((sum, c) => sum + c.tools.length, 0);
